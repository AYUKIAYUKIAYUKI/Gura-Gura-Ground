//============================================================================
// 
// ボム衝撃波、ヘッダーファイル [bombshockwave.h]
// Author : 福田歩希
// 
//============================================================================

#pragma once

//****************************************************
// インクルードファイル
//****************************************************
#include "API.collider.h"
#include "API.physics.object.h"

//****************************************************
// 前方宣言
//****************************************************
class CBomb;

//****************************************************
// ボム衝撃波クラスの定義
//****************************************************
class CBombShockWave : public CPhysicsObject
{
public:

	//****************************************************
	// special function
	//****************************************************
	CBombShockWave(OBJ::TYPE Type, OBJ::LAYER Layer); // デフォルトコンストラクタ
	~CBombShockWave() override;                       // デストラクタ

	//****************************************************
	// function
	//****************************************************

	// コライダーのファクトリ
	void FactoryCollider(Collision::SHAPETYPE Type, float fWidth = 1.0f, float fHeight = 1.0f, float fDepth = 1.0f);

	// 更新処理
	void Update() override;

	// 描画処理
	void Draw() override;

	// 期間の設定
	inline void SetDuration(int nDuration) { m_nDuration = nDuration; }

private:

	//****************************************************
	// data
	//****************************************************
	int m_nDuration; // 期間
};